...
gates=0; for gate in 'scripts/check-ios-camera-source.py --mode project' 'scripts/check-ios-camera-source.py --mode behavior' 'scripts/test_workflow_contract.py' 'scripts/test_credential_fixture_policy.py' 'scripts/test_native_suite_execution.py' 'scripts/test_frame_preprocessing_mutations.py' 'scripts/test_prediction_output_mutations.py' 'scripts/run-frame-preprocessing-tests.sh' 'scripts/run-prediction-range-tests.sh' 'scripts/run-prediction-output-tests.sh' 'scripts/run-ios-build.sh' 'scripts/test-makefile-root.sh'; do grep -Fq "$CHECKOUT/$gate" "$LOG" || { printf 'make check did not invoke %s\n' "$gate" >&2; exit 1; }; gates=$((gates+1)); done; [ "$gates" -eq 12 ]
...
printf '%s\n' 'Makefile root tests passed: 35 executed target/authority cases, 12 gate-invocation cases, 1 literal-dollar tool case, 1 raw tool Make-syntax rejection, 2 MAKEFILE_LIST rejections, 2 contained startup-boundary cases, and 10 mode-flag rejections'
